...
- **Packs** — progress popup with download size and extract `N / M` files plus per-file bytes. Cancel aborts the current download or extract. Store hits are reused. Failed entries are skipped. Unpack prefers installed **7-Zip** or Windows `tar` (sequential, native), then SHA-256 hashes the written files in parallel so Harvest still gets the same fingerprints. If neither tool is available, managed zip/7z extract writes first and hashes after.
