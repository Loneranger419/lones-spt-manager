...
Each profile junctions `BepInEx\config` onto `bepinex-config`. A new/empty profile used to get BepInEx defaults (**F1**, `HideManagerGameObject = false`), which breaks the F-key menu in EFT. Deploy seeds `com.bepis.bepinex.configurationmanager.cfg` with **F12** when missing, and forces `HideManagerGameObject = true` in `BepInEx.cfg`.

`BepInEx\plugins` itself stays a real folder because SPT-owned `spt\` lives there. Deploy junctions each extra plugin **subdirectory**. Loose `BepInEx/plugins/*.dll` packages are wrapped into `plugins/<name>/` first. Leftover files that belong at the game root (Dynamic Maps’ `EscapeFromTarkov_Data/Managed` DLLs, ReShade packs like Sharper Tushonka) are copied onto the install on deploy and removed when the mod is disabled. Copied files have the read-only bit cleared. If ReShade wrote `ReShade2.ini` (or a higher sidecar) with a further `TutorialProgress`, Deploy captures that into the profile’s `ReShade.ini` before purge, and Harvest promotes the sidecar onto the live `ReShade.ini` so the tutorial does not restart. Overwrite files under a **disabled** package’s `user/mods/<folder>` or `BepInEx/plugins/<folder>` are not staged (shared folders stay if another enabled mod still owns them). After overlays apply, empty real leftover folders under those roots are pruned so SPT does not try to load a hollow server-mod directory. Check `dir /AL BepInEx\plugins` for `<JUNCTION>` — files inside a junction look normal in Explorer.
...
- **The Forge** — search `sp-mod.com` (no token). Install downloads to `cache/forge/` then the store. **Import zip** is on this tab. `conflict: true` blocks. `fika_compatibility=incompatible` warns. Honour HTTP 429 / `Retry-After`. **SPT Mod Manager** (Forge id 2851) is incompatible with this app: search hides it, pack installs omit it, and Forge download is refused. What we call and how that matches their rules: [`FORGE.md`](FORGE.md).
- **Launch** — three buttons (**Solo**, **Fika host**, **Fika join**). Launch deploys the current profile first (idempotent if already deployed). Solo / Fika host start `SPT_Runtime\SPT.Server.exe`, snapshot `user\logs`, then wait for a **new** `Server has started` / `Started webserver` line (stale lines from earlier today do not count). TCP 6969 is only a fallback when no log folder exists. `SPT.Server` is started with stdin closed and Quick Edit off so a GUI-launched console does not freeze until Enter. The busy overlay reports Deploying → Starting server → Waiting for server → Starting launcher, then waits until that session’s server and/or client (`SPT.Server`, `SPT.Launcher`, `EscapeFromTarkov`) have all quit, then Harvest runs. **Fika join** starts the launcher only and writes `user\launcher\config.json` `Url` without dropping other keys. Never starts `EscapeFromTarkov.exe` or BattlEye. **Deploy** / **Harvest** / **Un-Deploy** sit under the installed list (Harvest is still there for sessions not started from this app). **Un-Deploy** is `DeployEngine.DetachAll` only (captures ReShade state, then the same detach Purge uses, without wiping manager data). Do not Harvest first: hashing the live tree through junctions is slow on a large pack and a process-lock miss skips detach entirely. Closing the main window does DetachAll when `settings.json` `undeployOnExit` is true (missing key = on). SPT running blocks Un-Deploy and close with a message. **Bind** sits with the game root. **Repair** is in Settings.
- **Leftovers** — right-click **Import leftover** to claim a real install folder into the store, then Deploy to junction it.
- **Purge manager data** — in Settings. Detaches junctions and wipes store / profiles / instances / cache / `settings.json`. Does **not** delete the SPT install. Leaves `LonesSptManager.exe` and `mods.json.example` if they sit in the manager data folder (same as the release zip / self-update target), and also leaves the running process file if that path is a direct child. Bind afterward.

**Settings** (header) picks theme (follow Windows, Dark, or Light), the manager data folder, Purge, Repair, **Remove junctions when the app closes**, **Check for updates**, and **Install update**. Theme and that close toggle are saved in `%AppData%\LonesSptManager\settings.json` (`theme`: `windows` / `dark` / `light`; `undeployOnExit`: default true). Windows color changes only apply when the choice is follow Windows. Unchecked store mods are dimmed in the list. On launch the app checks GitHub Releases and shows **App update** when `tag_name` is newer than `ProductInfo.Version`; that button installs and restarts. Default window is 1330×930. Branding lives in `assets/`: `lones-spt-manager-icon.png` (app mark) and `lones-spt-manager-card.png` (README hero). After changing the mark, run `tools/make-app-icon.ps1` to rebuild `src/Lones.SptManager.App/Assets/app.ico` (exe + window chrome). README screenshot is `src/Lones.SptManager.App/Assets/readme-main.png` (not embedded in the exe). Distribution is [GitHub Releases](https://github.com/Loneranger419/lones-spt-manager/releases), not a Forge listing. Forge thumbnails cache under `cache/thumbnails/`.
