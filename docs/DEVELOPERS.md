...
- **Launch** — three buttons (**Solo**, **Fika host**, **Fika join**). Solo / Fika host start `SPT_Runtime\SPT.Server.exe`, snapshot `user\logs`, then wait for a **new** `Server has started` / `Started webserver` line (stale lines from earlier today do not count). TCP 6969 is only a fallback when no log folder exists. `SPT.Server` is started with stdin closed and Quick Edit off so a GUI-launched console does not freeze until Enter. The busy overlay reports Starting server → Waiting for server → Starting launcher, then waits until that session’s server and/or client (`SPT.Server`, `SPT.Launcher`, `EscapeFromTarkov`) have all quit, then Harvest runs. **Fika join** starts the launcher only and writes `user\launcher\config.json` `Url` without dropping other keys. Never starts `EscapeFromTarkov.exe` or BattlEye. **Deploy** / **Harvest** sit under the installed list (Harvest is still there for sessions not started from this app). **Bind** sits with the game root. **Repair** is in Settings.
